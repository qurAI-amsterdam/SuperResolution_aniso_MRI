...
def load_data(data_config, limited_load=False, search_mask="*.nii.gz", **kwargs):
    ...
    images = {}
    if isnotebook:
        loop_generator = tqdm_notebook(files_to_load, desc='Load {}'.format(data_config.dataset))
    else:
        loop_generator = tqdm(files_to_load, desc='Load {}'.format(data_config.dataset))
    for file_name in loop_generator:
        if load_ref_labels:
        ...
        if all_frames:
            ...
            for sample_img, sample_lbl in img.all():
                sample = {'image': sample_img['image'],
                          ...
                          'spacing': sample_img['spacing'] if isinstance(sample_img['spacing'], np.ndarray) else np.array(sample_img['spacing']),
                          ...
                          'original_spacing': sample_lbl['original_spacing'] if isinstance(sample_img['original_spacing'], np.ndarray) else np.array(sample_img['original_spacing']),
        ...
        for filename in tqdm(files_to_load, desc="Load {} set".format(dataset)):
            ...
            if img.has_labels:
                ...
                for sample_img, sample_lbl in img.ed():
                    ...
                    if num_slices != sample_img['image'].shape[0]:
                        print("WARNING - {} #slices mismatch {} != {} ".format(sample_img['patient_id'], num_slices,
                                                                               sample_img['image'].shape[0]))
...
class ARVCImage(object):
    ...
    new_spacing = np.array([1, 1.25, 1.25])
    n_classes = 3
    ...
    def _return_all_phases(self):
        ...
        for frame_id in range(self.number_of_frames):
            ...
            # This is complete BULLSHIT! But we need to pass a valid object for this to the dataloader,
            # it won't accept None
            ...
            if self._resample or self.original_spacing[-1] < 1.:
                img, gt = self._do_resample(img, gt, self.original_spacing)
                # set new spacing
                # self.original_spacing[0] should contain the slice thickness! we don't change that.
                self._spacing = np.array([self.original_spacing[0], self.new_spacing[1], self.new_spacing[2]]).astype(np.float32)
    ...
    def _return_phase_images(self, cardiac_phase):
        # remember ARVC can have multiple phase IDs for ED/ES
        for frame_id, structures in sorted(self._info[cardiac_phase].items()):
            frame_id = int(frame_id)
            ignore_label = np.zeros(self.n_classes)
            if len(structures) == 1:
            ...
            if self._resample or self.original_spacing[-1] < 1.:
                ...
                # set new spacing
                self._spacing = np.array([self.original_spacing[0], self.new_spacing[1], self.new_spacing[2]]).astype(np.float32)
                # print("ARVCImage - resampled - ", self.original_spacing, self._spacing, frame_id, img.shape)
            # NOTE: 'structures' is a list of integers [0=BG, 1=LV, 2=RV] indicating the labels the volume contains
            if self._rescale:
                # print("ARVCImage - rescale")
                img = self._rescale_intensities(img, percentile=(1, 99))
            yield {'image': img.astype(np.float32), 'patient_id': self.patient_id, 'spacing': self._spacing,
                   'frame_id': frame_id, 'cardiac_phase': cardiac_phase, 'original_spacing': self.original_spacing,
                   'number_of_frames': self.number_of_frames, 'direction': self._direction, 'origin': self._origin}, \
                  {'labels': gt.astype(np.int16), 'patient_id': self.patient_id, 'spacing': self._spacing,
    ...
    def _do_resample(self, img, gt_lbl, current_spacing):
        img = apply_2d_zoom_3d(img, current_spacing, new_spacing=self.new_spacing, do_blur=True)
        gt_lbl = apply_2d_zoom_3d(gt_lbl, current_spacing, new_spacing=self.new_spacing,
                                  order=0, do_blur=False, as_type=np.int)
        return img, gt_lbl
