...
def compare_methods_slices(images_dict, pid, slice_id1, width=8, do_save=False, fig_name=None, row_height=4,
                           methods=None, slice1_x=None, slice1_y=None, axis=0, column_steps=2,
                           outdir="/home/jorg/images/", slice_id2=None, slice2_x=None, slice2_y=None,
                           exper_tag=None, frame1_id=None, frame2_id=None, transform=None,
                           upsample=None):
    ...
    num_columns = len(methods) * 2
    num_rows = 8 if slice_id2 is not None else 4
    height = row_height * (num_rows // 2)
    rsme = defaultdict(dict)
    fig = plt.figure(figsize=(width, height))
    # One row per method:
    # column1: Original, column2: method, column3, diff image
    row = 0
    col = 0
    for i, meth in enumerate(methods):
        slice_dict, plane = get_slices(pid, slice_id1, images_dict, axis,
                                       slice_x=slice1_x, slice_y=slice1_y, frame_id=frame1_id,
                                       transform=transform, upsample=upsample)
        if i == 0:
            ax1 = plt.subplot2grid((num_rows, num_columns), (0, 0), rowspan=2, colspan=2)
            ax1.imshow(slice_dict['orig'], cmap=cm.gray, interpolation='nearest', aspect='equal')
            ax1.set_title("Original", **title_font_small)
            ax1.axis("off")
        else:
            ax1 = plt.subplot2grid((num_rows, num_columns), (row, col), rowspan=2, colspan=2)
            ax1.imshow(slice_dict[meth], cmap=cm.gray, interpolation='nearest', aspect='equal')
            # ax2.set_xlabel("{}".format(methods[i]), ha='right', rotation=0, **title_font_small)
            ax1.set_title("{}".format(meth), **title_font_small)
            ax2 = plt.subplot2grid((num_rows, num_columns), (row+2, col), rowspan=2, colspan=2)
            diff, rsme[meth][slice_id1] = get_diff(slice_dict['orig'], slice_dict[methods[i]])
    ...
    if fig_name is None:
        if exper_tag is None:
            exper_tag = ""
        fig_name = str(pid) + "_" + plane + "_s" + str(slice_id1) + "_" + exper_tag + ".pdf"
    fig_name = os.path.join(outdir, fig_name)
    if do_save:
...
def plot_interpol_example(interpol_grid, orig_slices, width=14, height=8, do_save=False, do_show=True,
    ...
    alpha_range = np.linspace(0, 1, interpol_grid.shape[0])
    fig = plt.figure(figsize=(width, height))
    ...
    ax1.imshow(orig_slices[0], cmap=cm.gray, interpolation='nearest', aspect='equal')
    ax1.set_title("Original" "\n" "slice 1", ha='center', **title_font_small)
    ax1.xaxis.set_ticklabels([]), ax1.yaxis.set_ticklabels([])
    ax1.xaxis.set_ticks([]), ax1.yaxis.set_ticks([])
    ax1 = plt.subplot2grid((6, 6), (0, 2), rowspan=2, colspan=2)
    ax1.imshow(orig_slices[1], cmap=cm.gray, interpolation='nearest', aspect='equal')
    ax1.set_title("Original" "\n" "slice 2", ha='center', **title_font_small)
    ax1.xaxis.set_ticklabels([]), ax1.yaxis.set_ticklabels([])
    ax1.xaxis.set_ticks([]), ax1.yaxis.set_ticks([])
    ax1 = plt.subplot2grid((6, 6), (0, 4), rowspan=2, colspan=2)
    ax1.imshow(orig_slices[2], cmap=cm.gray, interpolation='nearest', aspect='equal')
...
def get_slices(pid, slice_id, images_dict, axis=0,
...
def interpolate_between_slices(trainer, z, num_interpol=3, side=None, dim=None):
    ...
    x_interp = trainer.decode(torch.FloatTensor(z_interp).to('cuda'))
