...
def vifvec_alternative(imref_batch, imdist_batch, do_rescale=False):
    if do_rescale:
        imref_batch = np.uint8(np.clip(imref_batch * 255., 0, 255))
        imdist_batch = np.uint8(np.clip(imdist_batch * 255., 0, 255))
    ...
    for a in range(batch_num):
        ...
        for i in range(len(subbands)):
            ...
            for j in range(len(lam)):
                temp1 += np.sum(np.log2(1 + np.divide(np.multiply(np.multiply(g, g), ss) * lam[j],
                                                      vv + sigma_nsq)))  # distorted image information
